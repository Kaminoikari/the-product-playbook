...
以下のオプションを提示：
- 🚀 **クイックモード** — 30分以内にアクションプランを作成（JTBD → PR-FAQ → North Star）
- 📦 **フルモード** — すべてのフレームワークを実行し、納品可能な企画ドキュメントを作成
- 🔄 **リビジョンモード** — 既存プロダクトの最適化（既存ユーザーとデータあり）
- ✏️ **カスタムモード** — フレームワークの組み合わせや完成度レベルを自由に選択
- ⚡ **ビルドモード** — 問題は既知、ディスカバリーをスキップしてソリューション設計へ直行
...
- 「既存プロダクトに機能を追加したい」「新機能を追加」→ ビルドモードを自動適用 → 機能拡張パス（4ステップ、`references/rules-build.md`を参照）
...
| 🔧 **機能拡張（ビルドモードの派生）** | 既存プロダクトに単一機能を追加；効率化された4ステップフロー | 問題 + コンテキスト → 3つの並行ソリューション + AI推奨 → リスク評価 → 実行スコープ | 既存プロダクトへの機能追加；明確な機能要件 |
...
2. **プロダクトコンテキストチェック**：プロジェクトディレクトリに`.product-context.md`が存在するか確認（ルールは`references/rules-context.md`参照）。
...
6. **品質セルフチェックは問題を表面化させる必要がある**：各ステップの品質チェックリストで、すべての項目が✅であってはなりません。すべてが合格した場合、Claudeは積極的に「この成果物の最も弱い点」を特定し、それを強化する方法を説明する必要があります。これは重箱の隅をつつくのではなく、セルフレビューメカニズムがゴム印を押すのではなく本当に機能していることを保証するためです。
