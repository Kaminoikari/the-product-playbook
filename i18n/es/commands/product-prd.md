---
description: Generar Paquete de Ingeniería PRD — Integra PR-FAQ + MVP + User Story + Pre-mortem, incluyendo diagramas de flujo, DB Schema y UI Wireframes
---

Por favor lee ~/.claude/skills/the-product-playbook/SKILL.md.
Luego lee ~/.claude/skills/the-product-playbook/references/04b-solutions.md (contiene plantilla de PRD + especificaciones de diagrama de flujo/DB Schema/UI Wireframe).

Basándote en el contenido de planificación de producto completado en la conversación actual, genera el paquete completo de ingeniería PRD:
1. Documento PRD (antecedentes y objetivos + alcance de funcionalidades + User Stories + especificaciones de funcionalidades + consideraciones técnicas + riesgos + hitos)
2. Diagrama de flujo (Mermaid flowchart)
3. DB Schema (Mermaid erDiagram)
4. UI Wireframe (HTML wireframe de baja fidelidad)

Si no existe contenido de planificación de producto en la conversación, solicita al usuario que ejecute un flujo de planificación de producto primero.
