...
다음 옵션을 제시하세요:
- 🚀 **퀵 모드** — 30분 이내에 실행 가능한 방향 도출 (JTBD → PR-FAQ → North Star)
- 📦 **풀 모드** — 모든 프레임워크를 거쳐 납품 가능한 기획서 작성
- 🔄 **리비전 모드** — 기존 제품 최적화 (기존 사용자와 데이터 보유)
- ✏️ **커스텀 모드** — 프레임워크 조합 또는 완성도 수준 직접 선택
- ⚡ **솔루션 직행 모드** — 문제가 명확한 경우 디스커버리를 건너뛰고 솔루션으로 바로 진행

빠른 트리거:
- "새로운 아이디어가 있는데 빠르게 검증하고 싶어요" → 퀵 모드 자동 적용
- "전체 제품 기획서를 작성하고 싶어요" → 풀 모드 자동 적용
- "무엇을 만들지 이미 알고 있어요" → 솔루션 직행 모드 자동 적용
- "제품을 개선해야 해요" → 리비전 모드 자동 적용
- "기존 제품에 기능을 추가하고 싶어요" 또는 "새 기능 추가" → 솔루션 직행 모드 자동 적용 → 기능 확장 경로 (4단계, `references/rules-build.md` 참조)
...
| ⚡ **솔루션 직행 모드** | 디스커버리 생략, 솔루션으로 바로 진행 | PR-FAQ + Pre-mortem + GEM/RICE + MVP + North Star | 문제가 명확한 경우; 빠른 실행 필요 |
| 🔧 **기능 확장 (솔루션 직행 변형)** | 기존 제품에 단일 기능 추가; 간소화된 4단계 플로우 | 문제 + 컨텍스트 → 3개 병렬 솔루션 + AI 추천 → 리스크 평가 → 실행 범위 | 기존 제품에 기능 추가; 기능 요구사항이 명확한 경우 |
...
| ⚡ 솔루션 직행 모드 | `references/rules-build.md` |
...
2. **제품 컨텍스트 점검**: 프로젝트 디렉토리에 `.product-context.md`가 존재하는지 확인하세요 (`references/rules-context.md`의 규칙 적용).
...
6. **품질 자체 점검은 문제를 표면화해야 합니다**: 각 단계의 품질 체크리스트에서 모든 항목이 ✅가 되어서는 안 됩니다. 모든 항목이 통과하면 Claude가 능동적으로 "이 산출물의 가장 약한 부분"을 식별하고 강화 방법을 설명해야 합니다. 이는 트집잡기가 아니라 자체 검토 메커니즘이 형식적이 아닌 실질적으로 작동하도록 보장하는 것입니다.
