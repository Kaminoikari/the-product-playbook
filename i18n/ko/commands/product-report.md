---
description: HTML 기획 보고서 생성 — 모든 제품 기획 내용을 오프라인 열람 가능한 단일 HTML 보고서로 통합
---

~/.claude/skills/the-product-playbook/SKILL.md와 references/06-html-report.md를 읽어 주세요.

현재 대화에서 완료된 제품 기획 내용을 기반으로, 06-html-report.md의 디자인 사양에 따라 전체 HTML 기획 보고서를 생성하세요:
- 단일 HTML 파일 (CSS + JS 인라인, Google Fonts CDN으로 Inter 로드)
- 완료된 단계만 동적으로 렌더링; 아직 완료되지 않은 단계는 건너뛰기
- 고정 목차 네비게이션, 카드 기반 레이아웃, 인터랙티브 효과 포함

대화에 제품 기획 내용이 없는 경우, 제품 기획 플로우를 먼저 실행하도록 안내하세요.
