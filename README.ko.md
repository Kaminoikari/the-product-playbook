...
- 📊 **다중 형식 출력** — HTML 보고서, PRD, PowerPoint 덱, 개발 핸드오프 패키지
...
3. `the-product-playbook/` 전체 폴더 업로드
...
```
다음 명령을 실행하여 the-product-playbook 스킬을 설치(또는 업데이트)하고,
완료되면 결과를 알려주세요:
...
| 명령 | 설명 |
|------|------|
| `/the-product-playbook` | 전체 제품 기획 가이드 플로우 시작 |
